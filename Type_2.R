...
## Parameters: beta, a = log(eta), zeta_p = log(Q_p)
## Methods: signed root r, modified root r*
###############################################

## 1. Type II log-likelihood in (beta, a)
...
## res_type2 must have columns:
## param, beta, eta, n, r_fail (or similar), conf_level, method, coverage

# Rename r_fail -> r and build factors
res_type2 <- res_type2 %>%
...
# Helper function to plot one confidence level
...
beta_length_signed <- function(x, n, conf_level = 0.95) {
  ...
  beta_hat <- mle_obj$beta_hat
  step <- max(0.01, beta_hat * 0.05)  # 步长约为 MLE 的 5%
...
## ============================================================================
## 4. 模拟长度（Type II 右删失，固定 n，变化 r_fail）
## ============================================================================
...
res_length <- sim_length_type2(
  ...
  r_vec     = c(3, 5, 8),
  p_quant   = 0.1,
  n_rep     = 1000,        # 可适当增加至 1000 或更高
  conf_level = 0.95,
  seed      = 123
)

save(res_length, file = "type2_buchang.Rdata")


load("type2_buchang.Rdata")
...
facet_grid2(
  param_lab ~ beta_eta,
  scales = "free_y",        # Y轴范围自由
  independent = "y",        # ✅ 每个子图真正独立Y轴（核心）
...
p_length <- ggplot(plot_data, aes(x = r_fail, y = avg_length,
  ...
  # 使用基本的 facet_grid，统一Y轴
  facet_grid(param_lab ~ beta_eta,
             scales = "fixed",  # 固定坐标轴，所有子图统一
             labeller = labeller(param_lab = label_parsed,
                                 beta_eta = label_parsed)) +
...
print(p_length)
# ggsave("weibull_typeII_avg_length.pdf", p_length, width = 7, height = 6)
