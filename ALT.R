...
# mu1_vec   <- -0.4245567
# sigma_true <- 0.22713
# beta_vec  <- 1 / sigma_true   # ≈ 4.403
# x_levels    <- c(28.84, 34.68, 38.02) * 0.1
# n_per_level <- c(6, 6, 6)
# x_use       <- 20 * 0.1
# conf_level  <- 0.95
# n_rep       <- 100              # 重复次数，可调高至 1000
# seed        <- 123
...
res_alt_all <- sim_alt_params_quantile(
  ...
  mu1_vec      = mu1_vec,
  x_levels     = x_levels,
  n_per_level  = n_per_level,   # total n = 10
  x_use        = x_use,
  p_quant      = 0.3,
  conf_level   = 0.95,
  n_rep        = 100,
...
###############################################################################
## 附加：区间长度计算与模拟（仅 signed & modified，步进求根）
###############################################################################
...
## ============================================================================
## 4. log 分位数 ζ_p (use stress) 的两种方法
## ============================================================================
...
save(res_len, file = "alt_buchang.Rdata")
